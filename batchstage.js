...
const rooms = ["empty2-real.jpg", "empty3-real.jpg"];

for (let i = 0; i < rooms.length; i++) {
  const file = rooms[i];
  const num = i + 2; // staged-2, staged-3 (test3 is effectively staged-1)
  console.log(`\n${file} -> staged-${num}.png`);

  const imgBuf = await readFile(file);
  const imgUri = `data:image/jpeg;base64,${imgBuf.toString("base64")}`;

  console.log("  masking floor + lower walls...");
  ...
  console.log("  staging full furniture set...");
  ...
  await writeFile(`staged-${num}.png`, genBuf);
  console.log(`  saved staged-${num}.png`);

  await sleep(10000);
}

console.log("\nDone! Compare empty2/3-real.jpg with staged-2/3.png");
